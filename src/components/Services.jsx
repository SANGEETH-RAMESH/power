          ...
          {services.map((s, i) => (
            <div key={i} className={`reveal reveal-delay-${i + 1} service-card`} style={{
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.borderColor = s.green ? 'rgba(90,140,46,.5)' : 'rgba(43,91,168,.5)'
              }}
              onMouseLeave={e => {
                e.currentTarget.style.transform = ''
                e.currentTarget.style.borderColor = s.green ? 'var(--line-g)' : 'var(--line)'
              }}
            >
              {/* gradient bg */}
              <div style={{
              ...
              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, letterSpacing: 2, color: 'var(--muted)', marginBottom: 28 }}>{s.num}</div>
              <div style={{
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 32px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {s.features.map((f, j) => (
                  <li key={j} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13, color: 'var(--muted)' }}>
                    <span style={{ color: 'var(--green-hi)', flexShrink: 0, fontFamily: "'Space Mono', monospace" }}>→</span>
                    {f}
                  </li>
                ))}
              </ul>
              <a href="#contact" style={{
                ...
                fontSize: 13, fontWeight: 500, letterSpacing: .5,
                color: s.green ? 'var(--green-hi)' : 'var(--blue-hi)',
                textDecoration: 'none',
                borderBottom: `1px solid ${s.green ? 'rgba(121,188,60,.3)' : 'rgba(74,127,212,.3)'}`,
                paddingBottom: 2, transition: 'color .2s, border-color .2s',
              }}
                onMouseEnter={e => { e.currentTarget.style.color = 'var(--white)'; e.currentTarget.style.borderColor = 'transparent' }}
                onMouseLeave={e => { e.currentTarget.style.color = s.green ? 'var(--green-hi)' : 'var(--blue-hi)'; e.currentTarget.style.borderColor = s.green ? 'rgba(121,188,60,.3)' : 'rgba(74,127,212,.3)' }}
              >{s.cta}</a>
            </div>
          ))}
        </div>
